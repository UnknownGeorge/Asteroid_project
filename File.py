...
class top_file:
    ...
    def writer(self, name, value):
        ...
        with open(self.file_location,"w") as files:
            file_read.update({len(file_read):{"name":name, "score":str(value)}})
            #files.truncate(0)
    ...
    def reader(self):
        with open(self.file_location,"r+") as files:
            ...
            sorted_list = sorted(sorted(file_read.items(), key=lambda x:x[1]['score'], reverse=True), key=lambda x: x[1]['name'])
            print(sorted_list)
            return sorted_list
